...
! CICE Copyright Notice

! Copyright 2010, LANSLLC. All rights reserved. Unless otherwise indicated,
! this information has been authored by an employee or employees of the Los
! Alamos National Security, LLC (LANS), operator of the Los Alamos National
! Laboratory under Contract No. DE-AC52-06NA25396 with the U.S. Department of
! Energy. The U.S. Government has rights to use, reproduce, and distribute
! this information. The public may copy and use this information without
...
! Local
      ...
      real (kind=dbl_kind) :: &
         ...
         p75ws    , &
         p5ws     , &
         recip_lm_mu  ! reciprocal of (1.0- lm*lm*gauspt(ng)*gauspt(ng))
...
!           gam = gamma(ws,mu0n,gs,lm)
           ...
           gam  = p5*ws*((c1 + c3*gs*(c1-ws)*mu0n*mu0n) &
                        / (c1-lm*lm*mu0n*mu0n))
           apg = alp + gam
           amg = alp - gam
           ...
           p5ws     = p5*ws
           gsc1ws   = c1 + gs*(c1-ws)
           c3gsc1ws = c3*gs*(c1-ws)
           do ng=1,ngmax
             mu  = gauspt(ng)
             recip_lm_mu = 1.0/(c1 - lm*lm*mu*mu)
             gwt = gauswt(ng)
             swt = swt + mu*gwt
             trn = max(exp_min, exp(-ts/(mu)))
!             alp = alpha(ws,mu,gs,lm)
!             gam = gamma(ws,mu,gs,lm)
             alp = p75ws*mu*((gsc1ws)*recip_lm_mu)
             gam = p5ws*((c1 + c3gsc1ws*mu*mu) *recip_lm_mu)
             apg = alp + gam
             amg = alp - gam
